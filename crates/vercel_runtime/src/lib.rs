    ...
    loop {
        ...
        tokio::task::spawn(async move {
            if let Err(_e) = http1::Builder::new()
                ...
                // FORK: enable HTTP/1.1 upgrades so WebSocket handshakes complete.
                // hyper now injects `hyper::upgrade::OnUpgrade` into request
                // extensions and drives the upgraded connection after the 101,
                // which axum's `WebSocketUpgrade` extractor relies on. The `end`
                // IPC message is already sent right after the handler returns the
                // 101 response (see above) — matching the Python runtime's
                // "end at websocket.accept" detached-upgrade flow.
                .with_upgrades()
                .await
            {
