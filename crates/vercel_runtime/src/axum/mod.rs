...
impl StreamingUtils {
    ...
    pub async fn process_response(
        ...
        // FORK: a `101 Switching Protocols` is a WebSocket handshake response —
        // it carries no body, and the upgraded socket is driven separately by
        // hyper (via `OnUpgrade`) once the 101 is written. Pass it through with an
        // empty body without running it through the streaming/`to_bytes` path.
